...
class ModelTrainerBuilder:
    ...
    def set_trainer_model(self):
        ...
        model = input("Select model: ").strip()  # Get user input and remove leading/trailing whitespace
    ...
    def set_specific_model(self):
        ...
        if trainer_model in trainer_config_creators:
            self.__model_config['specific_model'] = trainer_config_creators[trainer_model]()
        else:
    ...
    def set_test_size(self):
        ...
        while True:
            ...
            if 0.0 < test_size < 1.0:
                ...
                print("Invalid range. Please enter a value between 0.0 and 1.0 (exclusive).")
        return self
    ...
    def set_model_name(self):
        if self.__model_config['if_save_model']:
            ...
            loop = True
            while loop:
                model_name = input("Enter a custom model name (without extension): ").strip()
                # Sanitize user input
                if not model_name:
                ...
                if os.path.exists(model_path):
                    print(f"The file '{model_path}' already exists. Please choose a different name.")
                else:
    ...
    def set_metrics_name(self):
        if self.__model_config['if_save_metrics']:
                ...
                loop = True
                while loop:
                    metrics_name = input("Enter a custom metrics name (without extension): ").strip()
                    if not metrics_name:
                        print("Metrics name cannot be empty. Please try again.")
                        continue
                    metrics_path = os.path.join(f"{metrics_name}.txt")
                    if os.path.exists(metrics_path):
                        print(f"The file '{metrics_path}' already exists. Please choose a different name.")
                    else:
...
def create_model_trainer():
    ...
    while loop:
        ...
        model_config = MTB.set_trainer_model()\
                        .set_specific_model()\
                        .set_test_size()\
                        .set_random_seed()\
                        .set_if_scaler()\
